...
window.updateRenderedItem = (data) ->
  controller = angular.element('#assessmentTitle').controller()
  scope = angular.element('#assessmentTitle').scope()
  scope.$apply ->
    scope.hintButton = {
        title: "Show Hint",
        itemId: "24",
        id: "HINT"}
  console.log(angular.element('#assessmentTitle').scope().hintButton)
  if data.redirectUrl
